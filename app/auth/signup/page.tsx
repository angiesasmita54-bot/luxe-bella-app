    ...
    <div className="min-h-screen flex items-center justify-center bg-primary-50 py-8 px-4">
      <div className="bg-white p-8 md:p-10 rounded-lg shadow-lg w-full max-w-md border border-primary-200">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-primary-700 text-sm">Join Luxe Bella and start your beauty journey</p>
        </div>
        ...
        <form onSubmit={handleEmailSignUp} className="space-y-5">
          ...
          <div>
            <label className="block text-sm font-semibold mb-2 text-primary-700">
              Phone <span className="text-primary-400 font-normal">(Optional)</span>
            </label>
            <input
              type="tel"
              name="phone"
            ...
            <button
              onClick={async () => {
                try {
                  ...
                  if (result?.error) {
                    toast.error('Google sign-in failed. Please try again or use email sign-up.')
                  }
                } catch (error: any) {
                  console.error('Google sign-in error:', error)
                  toast.error('Google sign-in is not configured. Please use email sign-up or configure OAuth credentials.')
                }
              }}
            ...
            <button
              onClick={async () => {
                try {
                  ...
                  if (result?.error) {
                    toast.error('Facebook sign-in failed. Please try again or use email sign-up.')
                  }
                } catch (error: any) {
                  console.error('Facebook sign-in error:', error)
                  toast.error('Facebook sign-in is not configured. Please use email sign-up or configure OAuth credentials.')
                }
              }}
              className="w-full flex items-center justify-center gap-3 px-4 py-3 border-2 border-primary-300 rounded-lg hover:bg-primary-50 hover:border-primary-400 transition-all duration-200 font-medium text-primary shadow-sm hover:shadow-md"
            >
